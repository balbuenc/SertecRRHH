...
Partial Class man_funcionarios
    ...
    Protected Sub Button2_Click(ByVal sender As Object, ByVal e As System.EventArgs)
        ...
        Dim v_cod_funcionario As Label = CType(FormView1.Row.FindControl("cod_funcionarioLabel"), Label)
        Dim v_codigo As Label = CType(FormView1.Row.FindControl("codigoLabel"), Label)

        Dim v_apellidos As Label = CType(FormView1.Row.FindControl("apellidosLabel"), Label)
        Dim v_nombres As Label = CType(FormView1.Row.FindControl("nombresLabel"), Label)


        Try
            'Inserto el Registro en la tabla de mails
            ...
            ''Se establece la conexion
            conexion1 = New SqlConnection(mails_con.ConnectionString)
            conexion1.Open()

            comando1 = New SqlCommand()
            comando1.CommandType = Data.CommandType.Text
            comando1.Connection = conexion1
            comando1.CommandText = "INSERT INTO [dbo].[mails] " + _
           "([codigo],[funcionario],[email],[enabled],[cod_funcionario],[cod_gerencia]) " + _
            "VALUES(@codigo,@funcionario,@email,@enabled,@cod_funcionario,@cod_gerencia) "

            comando1.Parameters.AddWithValue("@codigo", v_codigo.Text)
            comando1.Parameters.AddWithValue("@funcionario", v_apellidos.Text + "," + v_nombres.Text)
            comando1.Parameters.AddWithValue("@email", "")
            comando1.Parameters.AddWithValue("@enabled", 1)
            comando1.Parameters.AddWithValue("@cod_funcionario", v_cod_funcionario.Text)
            comando1.Parameters.AddWithValue("@cod_gerencia", 11)
    ...
    Private Function HasEmail() As Boolean
        Dim v_cod_funcionario As Label = CType(FormView1.Row.FindControl("cod_funcionarioLabel"), Label)
        ...
        Try
            ...
            comando1.CommandType = Data.CommandType.Text
            comando1.Connection = conexion1
            comando1.CommandText = "select count(*) from mails where cod_funcionario = @cod_funcionario"

            comando1.Parameters.AddWithValue("@cod_funcionario", v_cod_funcionario.Text)

            Dim count As Integer
            count = comando1.ExecuteScalar()
            conexion1.Close()
    ...
    Protected Sub ImageButton5_Click(sender As Object, e As System.Web.UI.ImageClickEventArgs)
        Dim v_funcionario As Label = CType(FormView1.Row.FindControl("cod_funcionarioLabel"), Label)

        'se genera la url
        Dim url As String
        url = "man_funcionarios_historico.aspx?p_cod_empresa=1"

        url += "&p_cod_funcionario=" + v_funcionario.Text
